...
> 科研优先级：R1；对应 P4 兼容编号；是个人第一科研方向。必须晚于文本数据许可、PIT、去重和透明结构化基准，但不要求先完成完整 R2 ML 研究。

## 1. 中央问题

在透明传统/结构化基准与冻结的低搜索预算 evaluator 下，金融文本表示是否提供稳定、独立、成本后成立的样本外增量？
...
R1 期间固定 Linear/LightGBM evaluator；R1 比较的是表示，不开展广泛收益模型调参。完整 ML 模型比较留给 R2。
