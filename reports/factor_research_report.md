...
组合交易 score 采用严格滞后的 walk-forward 流程：训练窗口确定方向和候选因子，验证窗口检查方向一致性，下一调仓月固定权重测试。样本历史不足时，sample 模式可降级为简单均值以验证工程链路；real 模式若不能形成样本外 score 则直接失败。全样本 IC 只用于诊断展示，不参与同期交易权重。
...
组合构建使用月末调仓日期，默认按配置选择 Top50、单票上限 5%，并设置最小持仓数和单行业上限。当候选数量或约束组合不可行时，组合构建函数会抛出错误，而不是静默放松约束。

当前组合已支持单行业上限、最小持仓数、最大换手、最小成交额和成交额参与率；主动行业偏离作为归因结果输出。相对基准的逐股优化和更细现金目标仍属于后续真实组合优化范围。事件式回测输出订单、成交和持仓，并考虑下一交易日开盘执行、手数、涨跌停、停牌、资金不足和持仓延续，但样例数据仍不能代表真实盘口和成交深度。
...
样例 bundle 含合成基准，只用于验证严格日期对齐和相对指标计算；真实模式必须使用与股票池匹配的真实基准，日期不一致时不做隐式填充。若基准不匹配，例如用大盘指数评价小盘股票池，超额表现可能主要来自风格暴露而不是选股能力。
...
稳健性模块已覆盖滚动样本外窗口、零/标准/高成本、延迟 1/2/3 日和成交额参与率 1%/5%/10% 情景，并输出行业、市值、个股、回撤和成本贡献。仍需在真实数据上扩展调仓频率、股票池和参数网格，并防止把多轮敏感性检验反复用于挑选最终参数。

## LLM 辅助模块

LLM 事件模块的定位是辅助解释与事件标签结构化，不是直接生成交易指令。主线已合并离线标签客户端、prompt、schema、JSONL 缓存和人工抽查表；每条新标签保留原文、来源、发布时间、prompt/model 版本、原始 JSON、cache key 和创建时间。默认 rule-based dry-run，不调用外部 API；人工一致性低于阈值时事件因子不进入组合。
...
主要剩余风险包括：合成样例不能证明真实因子有效；真实历史成分、退市、复牌、新股和财务修订数据仍需验证；回测没有真实盘口深度和市场冲击校准；样本外、参数和容量框架虽已实现，但尚未在完整 PIT 数据上形成可信结论；LLM 标签工程具备抽查门槛，但仍缺合法真实文本源和已完成人工复核的标签集。
...
## 真实数据阶段 0–8 结论（2026-08-11）

本节为真实数据首批次（real-pit-20260725-r2）的阶段 0–8 运行结论，与前文合成样例结果严格区分；前文样例收益仍不构成任何有效性证据。数据缺口、登记豁免与签署记录索引：`reports/data_sources/预检总报告_20260727.md`（含 2026-08-11 口径修正附录）与 `reports/data_sources/signoff_sheet_20260727.md`（A/B/C 三部分签署）。

**数据门禁（阶段 0–1）**：7 张必需 PIT 表（RESSET 来源）加 4 张补导表通过预检、转换、签署与冻结；数据质量审计零阻断，PIT 时点、财务修订链、幸存者、历史成员覆盖、基准对齐五项专项审计全部通过。已知缺口（689009.SH 资金流/行业/财务、三个无来源因子输入）保持 NaN 并计数；两项字段覆盖率登记豁免逐日计数留痕。

**月度样本（阶段 2）**：118 个非重叠月度标签、31 个候选因子，构建门禁通过；2024-01-01 起最终留出期全程零接触。

**诊断与基准（阶段 3）**：真实诊断显示收益类序列平稳但普遍存在结构突变，成交/换手/离散度类序列存在强自相关与 ARCH 效应；ARIMAX、AR(1) 等复杂基准相对历史均值与滚动均值未表现出统计上显著的预测优势，按预注册验收标准不进入策略组合。

**动态方案（阶段 4–6，真实 OOS 57 个月）**：Kalman 动态因子权重 27 组预注册网格全部完成，但最优参数组在 IC 预测精度上未胜过 24 个月滚动 IC 基线（负结果如实记录）；Gaussian HMM（2/3 态、三种子）跨窗口状态稳定性不达标，有效概率月数 34 低于协议要求的 36；六类波动模型中无全面占优者（简单历史波动 QLIKE 最优但残差 ARCH 诊断最差，GARCH 族反之），按规则不自动晋级；DCC 因筛选后因子数 5 个低于下限 6 个而未估计。

**组合消融与晋级（阶段 7–8）**：受预注册 FDR（q≤0.05）约束，多数月份通过筛选的因子不足权重下限 5 个，动态权重只覆盖 4 个 OOS 月，七组消融的共同投资窗随之压缩为 4 个月，低于 36 个月门槛，七组方案全部判定 insufficient_history，独立增量无法归因。阶段 8 晋级审计据此得出最终结论 **insufficient_evidence**（dynamic_ready=false）。该结论的准确含义是"证据不足"：现有样本与预注册口径无法支持"动态方案有效"，也同样不能断言其无效；它不是对策略的否定，而是对当前证据强度的如实裁定。是否修订 FDR 口径或补充更长历史，属于需要重新签署的协议级决策。
