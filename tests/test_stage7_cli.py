...
from ashare_factor_research.main import _cmd_run_portfolio_ablation
from ashare_factor_research.time_series.stage7 import STAGE7_SCHEMAS


class Stage7CliTest(unittest.TestCase):
    ...
    def _args(self, root: Path, paths: dict[str, Path], mode: str) -> argparse.Namespace:
        ...
        stage46_dir = root / "stage46"
        stage46_dir.mkdir(exist_ok=True)
    ...
    def test_sample_cli_writes_fixed_schemas_and_engineering_marker(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(_cmd_run_portfolio_ablation(self._args(root, paths, "sample")), 0)
            ...
            summary = json.loads((output / "stage7_summary.json").read_text(encoding="utf-8"))
            self.assertTrue(summary["synthetic_engineering_only"])
            self.assertEqual(summary["status"], "insufficient_history")
            self.assertEqual(summary["command"], "run-portfolio-ablation")
            self.assertIn("monthly_ic", summary["input_hashes"])

    def test_real_cli_returns_nonzero_when_history_is_short(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            paths = self._write_inputs(root)
            self.assertEqual(_cmd_run_portfolio_ablation(self._args(root, paths, "real")), 2)
            summary = json.loads((root / "out_real" / "stage7_summary.json").read_text(encoding="utf-8"))
            self.assertNotEqual(summary["status"], "ablation_complete")
