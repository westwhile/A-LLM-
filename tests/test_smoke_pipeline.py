...
class SmokePipelineTest(unittest.TestCase):
    def test_sample_pipeline_runs_end_to_end(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((out_dir / "factor_inference.csv").exists())
            self.assertTrue((out_dir / "group_test_nonoverlap.csv").exists())
            self.assertTrue((out_dir / "execution_compliance.csv").exists())
            self.assertTrue((out_dir / "time_series_diagnostics.csv").exists())
            self.assertTrue((out_dir / "regime_probabilities.csv").exists())
            self.assertTrue((out_dir / "dynamic_factor_weights.csv").exists())
            self.assertTrue((out_dir / "volatility_forecasts.csv").exists())
            self.assertTrue((out_dir / "forecast_comparison.csv").exists())
            self.assertTrue((out_dir / "model_selection_audit.csv").exists())
            self.assertTrue((out_dir / "strategy_oos_comparison.csv").exists())
            self.assertEqual(result["score_source"], "synthetic_fallback")
    ...
    def test_standard_run_pipeline_writes_stage_outputs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((run_dir / "project_config_snapshot.yaml").exists())
            self.assertTrue((run_dir / "factor_config_snapshot.yaml").exists())
            self.assertTrue((run_dir / "backtest_config_snapshot.yaml").exists())
            ...
            self.assertTrue((run_dir / "figures" / "walk_forward_weights.csv").exists())
            self.assertTrue((run_dir / "figures" / "unfilled_order_analysis.csv").exists())
            self.assertTrue((run_dir / "figures" / "industry_return_attribution.csv").exists())
            self.assertTrue((run_dir / "figures" / "time_series_report.md").exists())
            self.assertTrue((run_dir / "figures" / "strategy_oos_comparison.csv").exists())
